...
endpoint and the shared **ShadowTools Plugins** connector.
...
2. Create the **ShadowTools Plugins** connector in ChatGPT using the displayed name,
   description and MCP URL. Keep the ShadowTools connection running.
...
5. A persistent reminder and **Open ChatGPT plugins** action stay visible above the installed list.
   Successful installation, configuration and tool-policy changes also display a reminder to refresh the **ShadowTools Plugins** connector in ChatGPT. Refresh the connector in ChatGPT after changing enabled tools, or enable ShadowTools's existing
...
- Pinned npm and Python recipes: Blender MCP, Knowledge Memory, Playwright Browser, Web Fetch and Unity Editor.
  Node.js/npm or Python/uv must be installed where the recipe requires them. ShadowTools installs
  ...
  searched, so installing uv there does not require restarting an already-running ShadowTools.
  For custom runtime locations, add the directory to PATH and restart ShadowTools.
...
is a third-party integration, not an official Blender feature supplied by ShadowTools.
...
ShadowTools's approved-folder sandbox. ShadowTools read-only mode refuses external plugin calls because upstream
...
ShadowTools remains MIT licensed. Settings provides the bundled Third-party Notices, generated from
...
External plugin licenses are shown per installation and remain separate from ShadowTools's own license.
