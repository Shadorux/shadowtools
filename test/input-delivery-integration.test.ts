...
describe('IPC input delivery and Goal control integration', () => {
  ...
  it('requires an exact plugin claim and matching schema before a refresh completion', async () => {
    ...
    publishPluginSurface('core', 'ShadowTools Core', 'test', 'Synthetic instructions', tools);
    ...
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'ShadowTools Core', tools })).body.ok).toBe(false);
    expect((await post('/plugin-refresh', { ...identity, action: 'claim', connectorName: 'ShadowTools Core', tools: [{ ...tools[0], description: 'Old declaration' }] })).body.ok).toBe(true);
  ...
  it('defaults automatic plugin refresh off and revokes an already offered claim without removing the backend', async () => {
    ...
    plugin.publishPluginSurface('core', 'ShadowTools Core', 'test', '', tools);
    ...
    const claim = { action: 'claim', id: saved.id, appId: 'asdk_app_off_on_test', connectorName: 'ShadowTools Core', tools: [{ ...tools[0], description: 'Older declaration' }] };
  ...
  it('accepts a manual plugin-refresh terminal state and removes it from browser pickup', async () => {
    ...
    const manual = await post('/plugin-refresh', { ...request, appId: 'asdk_app_synthetic', action: 'manual', connectorName: 'ShadowTools Core', tools: installed, error: 'Recreate or republish this custom app.' });
