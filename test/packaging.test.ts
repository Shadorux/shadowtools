...
describe('cross-platform packaging targets', () => {
  ...
  it('assembles every platform artifact in the reusable release workflow', () => {
    ...
    expect(workflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a /usr/bin/shadowtools');
    ...
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$mount_dir/ShadowTools.app\"");
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$zip_dir/ShadowTools.app\"");
    ...
    expect(workflow).toContain("grep -Fxq 'Name=ShadowTools' \"$desktop\"");
    expect(workflow).toContain("grep -Fxq 'Icon=shadowtools' \"$desktop\"");
    ...
    expect(debGui).toContain('xvfb-run -a /usr/bin/shadowtools');
  ...
  it('uses Noble-compatible Linux packages and a FUSE-independent AppImage runtime', () => {
    ...
    expect(builder.linux.artifactName).toBe('ShadowTools-Linux-${env.COS_PACKAGE_ARCH}.${ext}');
    ...
    expect(builder.linux.maintainer).toBe('Shadorux <shadoruxwebmaster@proton.me>');
    expect(pkg.desktopName).toBe('dev.shadorux.shadowtools.desktop');
    expect(pkg.homepage).toBe('https://github.com/Shadorux/shadowtools');
    ...
    expect(releaseWorkflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a /usr/bin/shadowtools');
    ...
    expect(releaseWorkflow).toContain("test \"$(dpkg-deb --field \"$deb\" Package)\" = shadowtools");
    ...
    expect(releaseWorkflow).toContain('test -L /usr/bin/shadowtools');
    expect(releaseWorkflow).toContain('installed_executable="$(readlink -f /usr/bin/shadowtools)"');
  ...
  it('pins the current macOS release to unsigned thin native bundles with explicit metadata checks', () => {
    ...
    expect(builder.mac.artifactName).toBe('ShadowTools-macOS-${arch}.${ext}');
    ...
    for (const marker of [
      ...
      "CFBundleExecutable: 'ShadowTools'",
      "CFBundleName: 'ShadowTools'",
      "CFBundleDisplayName: 'ShadowTools'",
  ...
  it('hides Electron helper parentheses from otool-classic without changing the inspected file', () => {
    const file = '/Applications/ShadowTools.app/Contents/Frameworks/ShadowTools Helper (GPU).app/Contents/MacOS/ShadowTools Helper (GPU)';
...
`;
  ...
  it('keeps version metadata aligned and validates artifacts independently of editorial release notes', () => {
    ...
    const artifacts = [
      'ShadowTools-Setup-x64.exe',
      'ShadowTools-Setup-arm64.exe',
      'ShadowTools-macOS-x64.dmg',
      'ShadowTools-macOS-x64.zip',
      'ShadowTools-macOS-arm64.dmg',
      'ShadowTools-macOS-arm64.zip',
      'ShadowTools-Linux-x64.AppImage',
      'ShadowTools-Linux-x64.deb',
      'ShadowTools-Linux-arm64.AppImage',
      'ShadowTools-Linux-arm64.deb',
      'ShadowTools-Extension.zip',
      'SHA256SUMS.txt'
    ];
