...
Ordinary pointer travel is also visibly interpolated over a short bounded duration. While native
pointer input is active, the Windows helper projects a short-lived click-through crimson halo at
the real pointer position and pulses it after clicks/drags. The halo never replaces the OS cursor
scheme and disappears with the helper process, so helper failure cannot strand a modified cursor.
