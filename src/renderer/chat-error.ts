...
export function chatErrorPresentation(error: ChatError, history: readonly SessionEvent[] = []) {
  ...
  let next = error.blocking === true
    ...
    : thinking
      ? t('You can send a follow-up. If recovery is eligible, the app may refresh the page, but it will not send another message for you.')
  ...
  if (repair && error.blocking !== true) next = `${repair.message.text} ${thinking
    ? t('You can send a follow-up. The refresh does not send another message for you.')
  ...
  if (continued) {
    ...
    next = repair || reloadPolicy ? `${next} ${work}` : t('Work continued after this notice. The failed page alone does not trigger another message.');
