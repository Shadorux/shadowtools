...
(() => {
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[ShadowTools: browser observation truncated to fit transport.]';
   ...
   *
   * Exact names, never a prefix: `ShadowTools Backup` would be somebody else's
  ...
  const OUR_CONNECTORS = [
    ...
    'ShadowTools Desktop',
    'ShadowTools Plugins',
    // Existing installations may still have connectors created under the pre-rebrand names.
  ...
  function controlState(input) {
    ...
    if (!connected) {
      return {
        mode: 'off',
        label: 'Compact',
        hint: 'ShadowTools is not running on this PC.',
        action: 'none'
      };
    }
    if (!conversationId) {
  ...
  function buildControl() {
    ...
    blocked.setAttribute(
      'data-clf-tip',
      'This chat is blocked in the ShadowTools app: its tool calls are refused and auto-compaction is off. To release it, open the app’s Chat tab, hover this chat in the sessions list and press its block symbol.'
    );
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', 'ShadowTools settings');
    root.hidden = true;
  ...
  function renderControl() {
    ...
    control.button.setAttribute('aria-label', 'ShadowTools settings');
  ...
  function replyError(reply) {
    ...
    if (reply.error === 'app_not_found') return 'ShadowTools is not running on this PC.';
