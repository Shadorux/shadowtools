...
let appExecutable;
if (targetPlatform === 'darwin') {
  const appBundle = path.join(packageRoot, 'ShadowTools.app');
  resourcesDir = path.join(appBundle, 'Contents', 'Resources');
  appExecutable = path.join(appBundle, 'Contents', 'MacOS', 'ShadowTools');
} else {
  resourcesDir = path.join(packageRoot, 'resources');
  appExecutable = path.join(packageRoot, targetPlatform === 'win32' ? 'ShadowTools.exe' : 'shadowtools');
}
